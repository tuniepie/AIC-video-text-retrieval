...
__all__ = ["load_model", "load_video", "tokenize", "load_scenes", "load_results"]
...
def load_results(video_path, start, end):
    """
    Given the video path, load 4 image from the video scene [start - end] to show results
    """
    video_reader = decord.VideoReader(video_path, num_threads=1, ctx=decord.cpu(0))
    decord.bridge.set_bridge('torch')
    video = video_reader.get_batch(np.linspace(start, end, 4, dtype=int).astype(np.intc)).byte()
    return video, np.linspace(start, end, 4, dtype=int)
